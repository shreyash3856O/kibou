...
router.get('/auth/me', async (req, res) => {
  ...
  if (!session) {
    session = await findOrRecoverSession(decoded.session_id, decoded.user_role || 'seeker', decoded.alias || 'Anonymous');
  }
...
router.post('/conversations/:id/accept', async (req, res) => {
  try {
    const { helper_session_id, helper_alias } = req.body;
    const conversation = findConversationById(req.params.id);
    if (!conversation) {
      return res.status(404).json({ error: 'Conversation not found' });
    }

    // Atomic Concurrency Guard: Ensure conversation is still waiting
    if (conversation.status !== 'waiting') {
      return res.status(409).json({ 
        error: 'This conversation has already been accepted by another helper.',
