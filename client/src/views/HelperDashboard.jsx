...
export default function HelperDashboard() {
  ...
  useEffect(() => {
    loadQueue();
    // Relaxed fallback timer to prevent server overload under high concurrency
    const interval = setInterval(loadQueue, 25000);

    const socket = getSocket();
    socket.on('new_seeker_in_queue', loadQueue);
    socket.on('queue_updated', loadQueue);
    socket.on('queue_item_claimed', ({ conversation_id }) => {
      setWaitingQueue((prev) => prev.filter((c) => c.conversation_id !== conversation_id));
    });

    return () => {
      clearInterval(interval);
      socket.off('new_seeker_in_queue', loadQueue);
      socket.off('queue_updated', loadQueue);
      socket.off('queue_item_claimed');
    };
  }, []);
  ...
  const handleToggleOnline = async () => {
    ...
    if (!session) {
      session = await initHelperSession();
    }
    if (session) {
      await api.updateHelperStatus(session.session_id, next).catch(() => {});
      const socket = getSocket();
  ...
  const handleAccept = async (convId) => {
    ...
    try {
      ...
      let res;
      try {
        res = await api.acceptConversation(convId, session.session_id, session.alias);
      } catch (err) {
        // If session was lost or 404, heal session and retry once
      ...
      socket.emit('helper_accepted_match', {
        ...
        helper_session_id: session.session_id,
        helper_alias: session.alias
      });
